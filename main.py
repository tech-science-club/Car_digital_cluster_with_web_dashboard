...
class Classic_style_dash_board(MDScreen):
	...
	def update_widget(self, dt):
		if int(self.val_pattern.val) > 50 and not self.scheduled and not self.scheduled_2:
			self.remove_widget(self.stock_box)
			self.add_widget(self.map, 4)
			self.scheduled = True
			self.scheduled_2 = True

		elif int(self.val_pattern.val) <= 50 and not self.scheduled and self.scheduled_2:
		...
		elif int(self.val_pattern.val) <= 50 and self.scheduled:
			self.scheduled = False
...
class Sport_style_dash_board(MDScreen):
	...
	def update_data(self, *args):
		...
		self.ids.rpm.text = str(round(Dash_Board.val * 1.5))
